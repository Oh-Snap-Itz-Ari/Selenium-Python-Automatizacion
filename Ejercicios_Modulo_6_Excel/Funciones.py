...
class Funciones_Globales (): # 2. Se crea la clase Funciones_Excel y se crea la inicialización dentro de esta
    ...
    def TextoMixto(self,tipo,selector,texto,tiempo):
        if (tipo == "XPATH"):
            ...
            try:
                val = WebDriverWait(self.driver, 5).until(EC.visibility_of_element_located((By.ID, selector)))  # 10. Busca el elemento
                val = self.driver.execute_script("arguments[0].scrollIntoView();",val)  # 11. Si lo encuentra va hacia el elemento
                val = self.driver.find_element(By.ID, (selector))  # 12. Selecciona el elemento a través de su XPath
    ...
    def FindExistMixto(self, tipo, selector, tiempo):
        if (tipo == "XPATH"):
            try:
                val = WebDriverWait(self.driver, 5).until(EC.visibility_of_element_located((By.XPATH, selector)))  # 13. Busca el elemento
                val = self.driver.execute_script("arguments[0].scrollIntoView();",val)  # 14. Si lo encuentra va hacia el elemento
